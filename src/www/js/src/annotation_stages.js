...
StageOneView.prototype = {
    ...
    create: function() {
        ...
        var button = $('<button>', {
            class: 'btn btn_start',
            text: 'HAGA CLICK PARA INICIAR UNA NUEVA ANOTACI&#211;N',
        });
...
StageTwoView.prototype = {
    ...
    create: function() {
        ...
        var button = $('<button>', {
            class: 'btn btn_stop',
            text: 'HAGA CLICK PARA TERMINAR LA ANOTACI&#211;N',
        });
        button.click(function () {
        ...
        if (this.wavesurfer.regions) {
            for (var region_id in this.wavesurfer.regions.list) {
                ...
                if (region.annotation === '' || (this.usingProximity && region.proximity === '')) {
                    if (this.usingProximity) {
                        Message.notifyAlert('Aseg&#250;rate de que todas tus anotaciones tengan una etiqueta de anotaci&#243;n y una etiqueta de proximidad!'); 
                    } else {
                        Message.notifyAlert('Aseg&#250;rate de que todas tus anotaciones tengan una etiqueta!'); 
                    }
                    return false;
                }
            }
    ...
    hint: function() {
        if (this.wavesurfer.regions && Object.keys(this.wavesurfer.regions.list).length === 1) {
            if (this.currentStage === 1 && !this.shownSelectHint) {
                ...
                Message.notifyHint('Haga doble clic en un segmento para seleccionarlo o anular su selecci&#243;n.');
            ...
            if (this.currentStage === 3 && !this.shownTagHint) {
                ...
                Message.notifyHint('Seleccione una etiqueta para anotar el segmento.');
